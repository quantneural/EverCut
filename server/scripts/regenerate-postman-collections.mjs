    ...
    },
    {
      ...
      ...request({
        ...
        body: formDataBody([
          ...
          textField('phoneNumber', '+15550000002'),
...
const readme = `# EverCut API - Postman Collections

## What Changed

These collections are now aligned to the same automation pattern used by \`13-barber-ratings.json\`:

- every collection has stable defaults and clearer run order
- dependent IDs are captured automatically from earlier requests
- important IDs are also mirrored into Postman globals so other collections can reuse them without copy-paste
- file-upload requests are prewired to \`server/postman-collections/assets/sample-upload.png\`
- payloads now match the real backend validators and response shapes
...
- \`test-customer-001\` / \`test.customer@example.com\` / \`+15550000001\`
- \`test-barber-001\` / \`test.barber@example.com\` / \`+15550000002\`

## One-Time Setup

1. Import every collection in this folder.
2. Import \`EverCut.postman_environment.json\`.
3. Select the \`EverCut Local\` environment in Postman.
4. Paste the customer token into \`customer_firebase_token\`.
5. Paste the barber token into \`barber_firebase_token\`.
6. Run requests directly. No per-collection token editing is needed.

## Token Setup

The included environment is now the intended place to set tokens once.

Collections read from these environment variables automatically:

- \`customer_firebase_token\`
- \`barber_firebase_token\`

Both are needed for the full route set because the backend enforces separate CUSTOMER and BARBER authorization.

## Recommended Order

Before running the customer discovery, booking, or rating collections, make sure the barber-side setup has created bookable data:

1. create the barber session
2. complete barber onboarding if needed
3. run the barber profile collection to capture the shop ID and keep the shop open
4. run the barber employees collection at least through the add/update requests
5. run the barber services collection at least through the add/update requests

Only after that should you run the customer discovery, booking, and rating flows.

### Customer-side

1. \`01-authentication.json\` -> \`1. Create Customer Session\`
2. \`02-onboarding.json\` -> \`1. Complete Customer Profile\` if the customer is new
...
2. \`02-onboarding.json\` -> \`2. Complete Barber Profile\` if the barber is new
3. \`07-barber-profile-shop.json\`
4. \`08-barber-employees.json\`
5. \`09-barber-services.json\`
6. \`11-barber-photos.json\`
7. \`10-barber-bookings.json\`
8. \`12-barber-earnings.json\`
9. \`13-barber-ratings.json\`

Requests marked \`(Terminal)\` are cleanup/destructive endpoints and should be run last.

## Shared Automation

The collections now set these Postman globals automatically when the backend returns them:
...
This is what lets the later collections run without manually copying IDs between requests or collections.

## Sample Upload Asset

The upload requests already point to:

\`\`\`
${SAMPLE_UPLOAD_PATH}
\`\`\`

That means customer onboarding, barber onboarding, profile picture, cover image, employee photo, and gallery-photo requests are immediately runnable on this machine after import.

## Known API State Constraints

The collections remove manual setup, but a few routes still depend on backend state that the public API cannot fully manufacture on demand:

- \`06-customer-ratings.json\` -> \`Add Rating\` succeeds once per customer per shop, then returns \`409\` on reruns because the API enforces one rating per customer/shop.
- \`10-barber-bookings.json\` -> \`Delete Paid Booking\` only succeeds when there is a booking with \`paymentStatus: success\`. The collection captures one automatically if it exists.
- \`13-barber-ratings.json\` still expects the dedicated seed data because reply add/update/delete/remove each need different rating states. If the marker IDs are missing, run:
...
## Backend-Correct Fixes Included

- booking times now use the API's required \`hh:mm AM/PM\` format
- customer booking reorder now sends \`date\` + \`time\` instead of unsupported keys
- customer booking update now sends all required fields
- service create/update bodies now match the real Joi schemas
- barber booking status filtering now defaults to \`confirmed\`, which matches current booking creation
- barber profile terminal actions are separated so \`Sign-Out Everywhere\` and \`Delete Account\` are not presented as a single sequential flow
- customer and barber onboarding defaults now match the Firebase test users and a shared location, so nearby search works out of the box
`;
...
writeFileSync(resolve(COLLECTIONS_DIR, 'README.md'), `${readme}\n`);
